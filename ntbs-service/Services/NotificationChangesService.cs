...
namespace ntbs_service.Services
{
    ...
    public class NotificationChangesService : INotificationChangesService
    {
        ...
        private IEnumerable<AuditLog> GetCanonicalLogs(List<AuditLog> group)
        {
            ...
            // Social risks page can contain sub-models if they are being set for the first time, which create their own audit entries
            var socialRisksUpdate = group.Find(log => log.EntityType == nameof(SocialRiskFactors));
            if (socialRisksUpdate != null || group.All(log => log.EntityType == nameof(RiskFactorDetails)))
            {
                return new List<AuditLog> {socialRisksUpdate ?? group.First()};
            }
